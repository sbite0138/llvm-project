//===-- MtGDisassembler.cpp - Disassembler for MtG ------------------===//
//
...
//===----------------------------------------------------------------------===//
//
// This file implements the MtGDisassembler class.
//
//===----------------------------------------------------------------------===//
...
#define DEBUG_TYPE "mtg-disassembler"

typedef MCDisassembler::DecodeStatus DecodeStatus;

namespace {
class MtGDisassembler : public MCDisassembler {
...
public:
  MtGDisassembler(const MCSubtargetInfo &STI, MCContext &Ctx)
...
static MCDisassembler *createMtGDisassembler(const Target &T,
                                                const MCSubtargetInfo &STI,
                                                MCContext &Ctx) {
  return new MtGDisassembler(STI, Ctx);
}

extern "C" LLVM_EXTERNAL_VISIBILITY void LLVMInitializeMtGDisassembler() {
  TargetRegistry::RegisterMCDisassembler(getTheMtGTarget(),
                                         createMtGDisassembler);
}

static const unsigned GR8DecoderTable[] = {
  MtG::PCB,  MtG::SPB,  MtG::SRB,  MtG::CGB,
  MtG::R4B,  MtG::R5B,  MtG::R6B,  MtG::R7B,
  MtG::R8B,  MtG::R9B,  MtG::R10B, MtG::R11B,
  MtG::R12B, MtG::R13B, MtG::R14B, MtG::R15B
};
...
#include "MtGGenDisassemblerTables.inc"
...
DecodeStatus MtGDisassembler::getInstructionI(MCInst &MI, uint64_t &Size,
...
DecodeStatus MtGDisassembler::getInstructionII(MCInst &MI, uint64_t &Size,
                                                  ArrayRef<uint8_t> Bytes,
                                                  uint64_t Address,
                                                  raw_ostream &CStream) const {
  uint64_t Insn = support::endian::read16le(Bytes.data());
...
static MtGCC::CondCodes getCondCode(unsigned Cond) {
  switch (Cond) {
  case 0: return MtGCC::COND_NE;
  case 1: return MtGCC::COND_E;
  case 2: return MtGCC::COND_LO;
  case 3: return MtGCC::COND_HS;
  case 4: return MtGCC::COND_N;
  case 5: return MtGCC::COND_GE;
  case 6: return MtGCC::COND_L;
  case 7: return MtGCC::COND_NONE;
  default:
    llvm_unreachable("Cond out of range");
  }
}

DecodeStatus MtGDisassembler::getInstructionCJ(MCInst &MI, uint64_t &Size,
                                                  ArrayRef<uint8_t> Bytes,
                                                  uint64_t Address,
                                                  raw_ostream &CStream) const {
  uint64_t Insn = support::endian::read16le(Bytes.data());
  ...
  if (Cond == 7)
    MI.setOpcode(MtG::JMP);
  else {
    MI.setOpcode(MtG::JCC);
...
DecodeStatus MtGDisassembler::getInstruction(MCInst &MI, uint64_t &Size,
                                                ArrayRef<uint8_t> Bytes,
                                                uint64_t Address,
                                                raw_ostream &CStream) const {
  if (Bytes.size() < 2) {
