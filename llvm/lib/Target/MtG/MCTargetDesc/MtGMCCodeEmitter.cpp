//===-- MtGMCCodeEmitter.cpp - Convert MtG code to machine code -----===//
//
...
//===----------------------------------------------------------------------===//
//
// This file implements the MtGMCCodeEmitter class.
//
//===----------------------------------------------------------------------===//
...
class MtGMCCodeEmitter : public MCCodeEmitter {
  MCContext &Ctx;
...
public:
  MtGMCCodeEmitter(MCContext &ctx, MCInstrInfo const &MCII)
      : Ctx(ctx), MCII(MCII) {}
...
void MtGMCCodeEmitter::encodeInstruction(const MCInst &MI,
...
unsigned MtGMCCodeEmitter::getMachineOpValue(const MCInst &MI,
                                                const MCOperand &MO,
                                                SmallVectorImpl<MCFixup> &Fixups,
                                                const MCSubtargetInfo &STI) const {
  if (MO.isReg())
  ...
  Fixups.push_back(MCFixup::create(Offset, MO.getExpr(),
      static_cast<MCFixupKind>(MtG::fixup_16_byte), MI.getLoc()));
  Offset += 2;
  return 0;
}

unsigned MtGMCCodeEmitter::getMemOpValue(const MCInst &MI, unsigned Op,
                                            SmallVectorImpl<MCFixup> &Fixups,
                                            const MCSubtargetInfo &STI) const {
  ...
  MtG::Fixups FixupKind;
  switch (Reg) {
  case 0:
    FixupKind = MtG::fixup_16_pcrel_byte;
    break;
  case 2:
    FixupKind = MtG::fixup_16_byte;
    break;
  default:
    FixupKind = MtG::fixup_16_byte;
    break;
...
unsigned MtGMCCodeEmitter::getPCRelImmOpValue(const MCInst &MI, unsigned Op,
                                                 SmallVectorImpl<MCFixup> &Fixups,
                                                 const MCSubtargetInfo &STI) const {
  ...
  Fixups.push_back(MCFixup::create(0, MO.getExpr(),
    static_cast<MCFixupKind>(MtG::fixup_10_pcrel), MI.getLoc()));
  return 0;
}

unsigned MtGMCCodeEmitter::getCGImmOpValue(const MCInst &MI, unsigned Op,
                                              SmallVectorImpl<MCFixup> &Fixups,
                                              const MCSubtargetInfo &STI) const {
  const MCOperand &MO = MI.getOperand(Op);
...
unsigned MtGMCCodeEmitter::getCCOpValue(const MCInst &MI, unsigned Op,
                                           SmallVectorImpl<MCFixup> &Fixups,
                                           const MCSubtargetInfo &STI) const {
  const MCOperand &MO = MI.getOperand(Op);
  assert(MO.isImm() && "Immediate operand expected");
  switch (MO.getImm()) {
  case MtGCC::COND_NE: return 0;
  case MtGCC::COND_E:  return 1;
  case MtGCC::COND_LO: return 2;
  case MtGCC::COND_HS: return 3;
  case MtGCC::COND_N:  return 4;
  case MtGCC::COND_GE: return 5;
  case MtGCC::COND_L:  return 6;
  default:
...
MCCodeEmitter *createMtGMCCodeEmitter(const MCInstrInfo &MCII,
                                         MCContext &Ctx) {
  return new MtGMCCodeEmitter(Ctx, MCII);
}

#include "MtGGenMCCodeEmitter.inc"
