...
MtGTargetLowering::MtGTargetLowering(const TargetMachine &TM,
  ...
  addRegisterClass(MVT::i8,  &MtG::GR8RegClass);
  addRegisterClass(MVT::i16, &MtG::GR16RegClass);

  // Compute derived properties from the register classes
  computeRegisterProperties(STI.getRegisterInfo());

  // Provide all sorts of operation actions
  setStackPointerRegisterToSaveRestore(MtG::SP);
  ...
  if (STI.hasHWMult16()) {
    ...
    setLibcallCallingConv(RTLIB::MUL_I64, CallingConv::MtG_BUILTIN);
  }

  // Several of the runtime library functions use a special calling conv
  setLibcallCallingConv(RTLIB::UDIV_I64, CallingConv::MtG_BUILTIN);
  setLibcallCallingConv(RTLIB::UREM_I64, CallingConv::MtG_BUILTIN);
  setLibcallCallingConv(RTLIB::SDIV_I64, CallingConv::MtG_BUILTIN);
  setLibcallCallingConv(RTLIB::SREM_I64, CallingConv::MtG_BUILTIN);
  setLibcallCallingConv(RTLIB::ADD_F64, CallingConv::MtG_BUILTIN);
  setLibcallCallingConv(RTLIB::SUB_F64, CallingConv::MtG_BUILTIN);
  setLibcallCallingConv(RTLIB::MUL_F64, CallingConv::MtG_BUILTIN);
  setLibcallCallingConv(RTLIB::DIV_F64, CallingConv::MtG_BUILTIN);
  setLibcallCallingConv(RTLIB::OEQ_F64, CallingConv::MtG_BUILTIN);
  setLibcallCallingConv(RTLIB::UNE_F64, CallingConv::MtG_BUILTIN);
  setLibcallCallingConv(RTLIB::OGE_F64, CallingConv::MtG_BUILTIN);
  setLibcallCallingConv(RTLIB::OLT_F64, CallingConv::MtG_BUILTIN);
  setLibcallCallingConv(RTLIB::OLE_F64, CallingConv::MtG_BUILTIN);
  setLibcallCallingConv(RTLIB::OGT_F64, CallingConv::MtG_BUILTIN);
...
SDValue MtGTargetLowering::LowerOperation(SDValue Op,
...
bool MtGTargetLowering::shouldAvoidTransformToShift(EVT VT,
                                                       unsigned Amount) const {
  return !(Amount == 8 || Amount == 9 || Amount<=2);
}

// Implemented to verify test case assertions in
// tests/codegen/mtg/shift-amount-threshold-b.ll
bool MtGTargetLowering::isLegalICmpImmediate(int64_t Immed) const {
  if (MtGNoLegalImmediate)
    return Immed >= -32 && Immed < 32;
  return TargetLowering::isLegalICmpImmediate(Immed);
}

//===----------------------------------------------------------------------===//
//                       MtG Inline Assembly Support
//===----------------------------------------------------------------------===//

/// getConstraintType - Given a constraint letter, return the type of
/// constraint it is for this target.
TargetLowering::ConstraintType
MtGTargetLowering::getConstraintType(StringRef Constraint) const {
...
MtGTargetLowering::getRegForInlineAsmConstraint(
  ...
  if (Constraint.size() == 1) {
    ...
    case 'r':   // GENERAL_REGS
      if (VT == MVT::i8)
        return std::make_pair(0U, &MtG::GR8RegClass);

      return std::make_pair(0U, &MtG::GR16RegClass);
    }
  }
...
static void AnalyzeArguments(CCState &State,
  ...
  bool Builtin = (State.getCallingConv() == CallingConv::MtG_BUILTIN);
  if (Builtin) {
  ...
  for (unsigned i = 0, e = ArgsParts.size(); i != e; i++) {
    ...
    if (!UsedStack && Parts == 2 && RegsLeft == 1) {
      ...
      UsedStack = true;
      CC_MtG_AssignStack(ValNo++, ArgVT, LocVT, LocInfo, ArgFlags, State);
...
SDValue
MtGTargetLowering::LowerCall(TargetLowering::CallLoweringInfo &CLI,
  ...
  // MtG target does not yet support tail call optimization.
  isTailCall = false;

  switch (CallConv) {
  default:
    report_fatal_error("Unsupported calling convention");
  case CallingConv::MtG_BUILTIN:
  case CallingConv::Fast:
  case CallingConv::C:
...
SDValue MtGTargetLowering::LowerCCCArguments(
    SDValue Chain, CallingConv::ID CallConv, bool isVarArg,
    const SmallVectorImpl<ISD::InputArg> &Ins, const SDLoc &dl,
    SelectionDAG &DAG, SmallVectorImpl<SDValue> &InVals) const {
  ...
  for (unsigned i = 0, e = ArgLocs.size(); i != e; ++i) {
    ...
    if (VA.isRegLoc()) {
      ...
      case MVT::i16:
        Register VReg = RegInfo.createVirtualRegister(&MtG::GR16RegClass);
...
bool
MtGTargetLowering::CanLowerReturn(CallingConv::ID CallConv,
                                     MachineFunction &MF,
                                     bool IsVarArg,
                                     const SmallVectorImpl<ISD::OutputArg> &Outs,
...
MtGTargetLowering::LowerReturn(SDValue Chain, CallingConv::ID CallConv,
  ...
  unsigned Opc = (CallConv == CallingConv::MtG_INTR ?
                  MtGISD::RETI_GLUE : MtGISD::RET_GLUE);
...
SDValue MtGTargetLowering::LowerCCCCallTo(
...
SDValue MtGTargetLowering::LowerShifts(SDValue Op,
                                          SelectionDAG &DAG) const {
  ...
  if (Opc == ISD::SRL && ShiftAmount) {
    ...
    // srl A, 1 => clrc; rrc A
    Victim = DAG.getNode(MtGISD::RRCL, dl, VT, Victim);
    ShiftAmount -= 1;
  }

  while (ShiftAmount--)
    Victim = DAG.getNode((Opc == ISD::SHL ? MtGISD::RLA : MtGISD::RRA),
                         dl, VT, Victim);
...
SDValue MtGTargetLowering::LowerGlobalAddress(SDValue Op,
  ...
  return DAG.getNode(MtGISD::Wrapper, SDLoc(Op), PtrVT, Result);
}

SDValue MtGTargetLowering::LowerExternalSymbol(SDValue Op,
                                                  SelectionDAG &DAG) const {
  SDLoc dl(Op);
  ...
  return DAG.getNode(MtGISD::Wrapper, dl, PtrVT, Result);
}

SDValue MtGTargetLowering::LowerBlockAddress(SDValue Op,
                                                SelectionDAG &DAG) const {
  SDLoc dl(Op);
...
static SDValue EmitCMP(SDValue &LHS, SDValue &RHS, SDValue &TargetCC,
  ...
  MtGCC::CondCodes TCC = MtGCC::COND_INVALID;
  switch (CC) {
  default: llvm_unreachable("Invalid integer condition!");
  case ISD::SETEQ:
    TCC = MtGCC::COND_E;     // aka COND_Z
  ...
  return DAG.getNode(MtGISD::CMP, dl, MVT::Glue, LHS, RHS);
}


SDValue MtGTargetLowering::LowerBR_CC(SDValue Op, SelectionDAG &DAG) const {
  ...
  return DAG.getNode(MtGISD::BR_CC, dl, Op.getValueType(),
                     Chain, Dest, TargetCC, Flag);
}

SDValue MtGTargetLowering::LowerSETCC(SDValue Op, SelectionDAG &DAG) const {
  ...
  if (Convert) {
    SDValue SR = DAG.getCopyFromReg(DAG.getEntryNode(), dl, MtG::SR,
                                    MVT::i16, Flag);
    if (Shift)
    ...
    return DAG.getNode(MtGISD::SELECT_CC, dl, Op.getValueType(), Ops);
  }
}

SDValue MtGTargetLowering::LowerSELECT_CC(SDValue Op,
                                             SelectionDAG &DAG) const {
  SDValue LHS    = Op.getOperand(0);
  SDValue RHS    = Op.getOperand(1);
...
SDValue MtGTargetLowering::LowerSIGN_EXTEND(SDValue Op,
                                               SelectionDAG &DAG) const {
...
SDValue
MtGTargetLowering::getReturnAddressFrameIndex(SelectionDAG &DAG) const {
  MachineFunction &MF = DAG.getMachineFunction();
  MtGMachineFunctionInfo *FuncInfo = MF.getInfo<MtGMachineFunctionInfo>();
...
SDValue MtGTargetLowering::LowerRETURNADDR(SDValue Op,
                                              SelectionDAG &DAG) const {
...
SDValue MtGTargetLowering::LowerFRAMEADDR(SDValue Op,
                                             SelectionDAG &DAG) const {
  MachineFrameInfo &MFI = DAG.getMachineFunction().getFrameInfo();
...
SDValue MtGTargetLowering::LowerVASTART(SDValue Op,
                                           SelectionDAG &DAG) const {
  MachineFunction &MF = DAG.getMachineFunction();
  MtGMachineFunctionInfo *FuncInfo = MF.getInfo<MtGMachineFunctionInfo>();
...
SDValue MtGTargetLowering::LowerJumpTable(SDValue Op,
                                             SelectionDAG &DAG) const {
    JumpTableSDNode *JT = cast<JumpTableSDNode>(Op);
    EVT PtrVT = Op.getValueType();
    SDValue Result = DAG.getTargetJumpTable(JT->getIndex(), PtrVT);
    return DAG.getNode(MtGISD::Wrapper, SDLoc(JT), PtrVT, Result);
}

/// getPostIndexedAddressParts - returns true by value, base pointer and
/// offset pointer and addressing mode by reference if this node can be
/// combined with a load / store to form a post-indexed load / store.
bool MtGTargetLowering::getPostIndexedAddressParts(SDNode *N, SDNode *Op,
                                                      SDValue &Base,
                                                      SDValue &Offset,
...
const char *MtGTargetLowering::getTargetNodeName(unsigned Opcode) const {
  ...
  case MtGISD::FIRST_NUMBER:       break;
  case MtGISD::RET_GLUE:           return "MtGISD::RET_GLUE";
  case MtGISD::RETI_GLUE:          return "MtGISD::RETI_GLUE";
  case MtGISD::RRA:                return "MtGISD::RRA";
  case MtGISD::RLA:                return "MtGISD::RLA";
  case MtGISD::RRC:                return "MtGISD::RRC";
  case MtGISD::RRCL:               return "MtGISD::RRCL";
  case MtGISD::CALL:               return "MtGISD::CALL";
  case MtGISD::Wrapper:            return "MtGISD::Wrapper";
  case MtGISD::BR_CC:              return "MtGISD::BR_CC";
  case MtGISD::CMP:                return "MtGISD::CMP";
  case MtGISD::SETCC:              return "MtGISD::SETCC";
  case MtGISD::SELECT_CC:          return "MtGISD::SELECT_CC";
  case MtGISD::DADD:               return "MtGISD::DADD";
  }
  return nullptr;
}

bool MtGTargetLowering::isTruncateFree(Type *Ty1,
                                          Type *Ty2) const {
...
bool MtGTargetLowering::isTruncateFree(EVT VT1, EVT VT2) const {
...
bool MtGTargetLowering::isZExtFree(Type *Ty1, Type *Ty2) const {
  // MtG implicitly zero-extends 8-bit results in 16-bit registers.
  return false && Ty1->isIntegerTy(8) && Ty2->isIntegerTy(16);
}

bool MtGTargetLowering::isZExtFree(EVT VT1, EVT VT2) const {
  // MtG implicitly zero-extends 8-bit results in 16-bit registers.
...
MachineBasicBlock *
MtGTargetLowering::EmitShiftInstr(MachineInstr &MI,
  ...
  case MtG::Rrcl8:
  case MtG::Rrcl16: {
    BuildMI(*BB, MI, dl, TII.get(MtG::BIC16rc), MtG::SR)
      .addReg(MtG::SR).addImm(1);
    Register SrcReg = MI.getOperand(1).getReg();
    Register DstReg = MI.getOperand(0).getReg();
    unsigned RrcOpc = MI.getOpcode() == MtG::Rrcl16
                    ? MtG::RRC16r : MtG::RRC8r;
  ...
  Register ShiftAmtReg = RI.createVirtualRegister(&MtG::GR8RegClass);
  Register ShiftAmtReg2 = RI.createVirtualRegister(&MtG::GR8RegClass);
  ...
  // cmp 0, N
  // je RemBB
  BuildMI(BB, dl, TII.get(MtG::CMP8ri))
    .addReg(ShiftAmtSrcReg).addImm(0);
  BuildMI(BB, dl, TII.get(MtG::JCC))
    .addMBB(RemBB)
    .addImm(MtGCC::COND_E);
  ...
  BuildMI(LoopBB, dl, TII.get(MtG::PHI), ShiftReg)
    .addReg(SrcReg).addMBB(BB)
    .addReg(ShiftReg2).addMBB(LoopBB);
  BuildMI(LoopBB, dl, TII.get(MtG::PHI), ShiftAmtReg)
    .addReg(ShiftAmtSrcReg).addMBB(BB)
    .addReg(ShiftAmtReg2).addMBB(LoopBB);
  if (ClearCarry)
    BuildMI(LoopBB, dl, TII.get(MtG::BIC16rc), MtG::SR)
      .addReg(MtG::SR).addImm(1);
  if (Opc == MtG::ADD8rr || Opc == MtG::ADD16rr)
    ...
    BuildMI(LoopBB, dl, TII.get(Opc), ShiftReg2)
      .addReg(ShiftReg);
  BuildMI(LoopBB, dl, TII.get(MtG::SUB8ri), ShiftAmtReg2)
    .addReg(ShiftAmtReg).addImm(1);
  BuildMI(LoopBB, dl, TII.get(MtG::JCC))
    .addMBB(LoopBB)
    .addImm(MtGCC::COND_NE);

  // RemBB:
  // DestReg = phi [%SrcReg, BB], [%ShiftReg, LoopBB]
  BuildMI(*RemBB, RemBB->begin(), dl, TII.get(MtG::PHI), DstReg)
    .addReg(SrcReg).addMBB(BB)
    .addReg(ShiftReg2).addMBB(LoopBB);
...
MtGTargetLowering::EmitInstrWithCustomInserter(MachineInstr &MI,
  ...
  if (Opc == MtG::Shl8  || Opc == MtG::Shl16 ||
      Opc == MtG::Sra8  || Opc == MtG::Sra16 ||
      Opc == MtG::Srl8  || Opc == MtG::Srl16 ||
      Opc == MtG::Rrcl8 || Opc == MtG::Rrcl16)
    return EmitShiftInstr(MI, BB);

  const TargetInstrInfo &TII = *BB->getParent()->getSubtarget().getInstrInfo();
  DebugLoc dl = MI.getDebugLoc();

  assert((Opc == MtG::Select16 || Opc == MtG::Select8) &&
  ...
  //  ...
  BB = copy1MBB;
  BuildMI(*BB, BB->begin(), dl, TII.get(MtG::PHI), MI.getOperand(0).getReg())
