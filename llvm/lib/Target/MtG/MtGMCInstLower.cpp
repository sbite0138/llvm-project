//===-- MtGMCInstLower.cpp - Convert MtG MachineInstr to an MCInst --===//
//
...
//===----------------------------------------------------------------------===//
//
// This file contains code to lower MtG MachineInstrs to their corresponding
// MCInst records.
//
//===----------------------------------------------------------------------===//
...
MCSymbol *MtGMCInstLower::GetJumpTableSymbol(const MachineOperand &MO) const {
  ...
  raw_svector_ostream(Name)
      << Ctx.getAsmInfo()->getPrivateLabelPrefix() << "JTI" << Printer.getFunctionNumber()
      << '_' << MO.getIndex();
...
MtGMCInstLower::GetConstantPoolIndexSymbol(const MachineOperand &MO) const {
  ...
  SmallString<256> Name;
  raw_svector_ostream(Name)
      << Ctx.getAsmInfo()->getPrivateLabelPrefix() << "CPI" << Printer.getFunctionNumber()
      << '_' << MO.getIndex();
